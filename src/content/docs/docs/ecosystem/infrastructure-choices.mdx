...
description: "Why Stardust's cloud-side defaults are Cloudflare R2 + Fly.io + Neon + Clerk + Plausible + GlitchTip — and why the architecture stays provider-agnostic."
---
...
| Auth | **Clerk** (or **Ory** if self-hosted) | Managed identity with social login, MFA, session management |
...
Both are supported by the architecture. The user running this workspace is AWS-certified — operating Stardust's cloud side on AWS would be straightforward. The reason we don't default there:

- AWS egress is expensive enough to threaten "free content stays free"
- Managed services on AWS are more configuration-heavy than the defaults above
- The default stack costs an order of magnitude less to run for low-volume workloads

For a self-hosted enterprise instance, AWS or GCP is often the right answer. The catalog server, license server, and content delivery can each be swapped independently.
