...
- **Device identity lives on the rig component; node-level `hardwareBinding` is the interim/fallback model** — rig basics pulled forward from v0.10.0 to v0.6.0 as rig-lite ([stardust-pit#122](https://github.com/StardustMT/stardust-pit/issues/122)): configure a device once in Setup → Rig, patch source nodes reference the component (`rigComponentId`), resolution falls back node → component → device so #2-era shows keep working. Rationale: #5/#7 already assumed a rig surface, and per-patch raw bindings accrue migration debt. #2's engine matching/filter/rebind plumbing is unchanged by this. *(Direction locked 2026-07-06 during post-#118 review; details lock in the #122 refinement session.)*
