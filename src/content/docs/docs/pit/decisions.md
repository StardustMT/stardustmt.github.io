...
- **MIDI device identity = midir's opaque platform port id, name as replug fallback** — not `(vendor_id, product_id)` as originally locked in #2 refinement: midir exposes no vendor/product IDs on any platform, so that decision was unimplementable as written. Port ids are stable on CoreMIDI (macOS); the display-name fallback covers replug on platforms with unstable ids. Names remain display-only + the open key. *(Amended 2026-07-03 while shipping [stardust-pit#2](https://github.com/StardustMT/stardust-pit/issues/2) in PR #118.)*
- **Device change decision tree: identity → rebind, sample-rate/buffer-size → rebuild** — `engine_rebind_routing` swaps cpal/midir I/O under the live Plan (no plugin reloads, held voices intact); anything that resizes buffers or reactivates plugins goes through a full `engine_start_from_patch` rebuild. On rebind failure the previous device stays (or is restored) active — never silent-fail, never silence. *(Shipped 2026-07-03, [stardust-pit#1](https://github.com/StardustMT/stardust-pit/issues/1) / PR #118.)*
- **Panic resets every controller the engine touches, not just notes** — sustain-off, per-tracked-voice note-offs + poly-aftertouch-clear, all-notes-off, pitch-bend center, mod-wheel 0, channel-pressure 0, all 16 channels, within one audio block. *(Shipped 2026-07-03, [stardust-pit#3](https://github.com/StardustMT/stardust-pit/issues/3) / PR #118.)*
- **Source kinds imply MIDI event classes** — a sustain-pedal source accepts only CC 64, expression CC 11, mod wheel CC 1, pitch wheel pitch-bend, pads notes, keyboards everything; binding filters (channel / note / CC range) narrow further. Prevents note-doubling when a pedal and a keyboard feed the same instrument. *(Locked while shipping #2, 2026-07-03.)*
- **Device identity lives on the rig component; node-level `hardwareBinding` ceases to exist** — rig basics pulled forward from v0.10.0 to v0.6.0 as rig-lite ([stardust-pit#122](https://github.com/StardustMT/stardust-pit/issues/122)): configure a device once in Setup → Rig, patch source nodes reference the component (`rigComponentId`) — that reference is the node's *identity*, not a binding override. There is **no fallback model**: a v2→v3 raw-JSON migration auto-converts #2-era per-node `hardwareBinding` blobs into rig components (one per distinct device+channel), points the nodes at them, and deletes the field. A source node with no component assigned is **silent** (UI flags it), not any-device omni. #2's engine matching/filter/rebind plumbing is reused underneath, resolving component bindings instead of node blobs. *(Direction locked 2026-07-06 during post-#118 review; fallback model dropped and details locked 2026-07-06 during #122 refinement.)*
- **Engine opens the union of rig-bound devices, session-wide** — MIDI input set derives from the rig (every device any component is bound to), not from the active patch. Patch switches never touch MIDI I/O; rebind (#1 path) fires only on rig edits and device connect/disconnect. Learn mode temporarily opens all connected inputs, then restores the rig-derived set. Supersedes [#121](https://github.com/StardustMT/stardust-pit/issues/121)'s draft "derive from the active patch's bindings". *(Locked 2026-07-06 during #122 refinement.)*
- **Sine synth → `instrument.testtone`** — hidden from catalog, diagnostic-only. Default built-in instrument becomes the bundled GM piano SFZ in v0.14.0. *(Shipped 2026-07-03 in v0.6.0 via [stardust-pit#9](https://github.com/StardustMT/stardust-pit/issues/9): `stardust.patch` + `stardust.show` schemas bumped to v2 with a v1→v2 rename migration; Settings → "Run engine self-test" is the only user surface. Engine topo lifts MIDI sources before any consumer in the same block — needed for the single-block end-to-end self-test render.)*
