...
Each button/switch has one configured action:
...
| **Start Transport** | Begin transport playback (v0.13.0 backing tracks) |
...
| **Send MIDI Message** | Send a named MIDI message out (forScore page turns, etc.) |
...
| **Custom Macro** | Fire a configured extension-API command (v0.15.0) |

Adding new actions is a v0.15.0 extension-API surface. Shipped actions cover the common cases for v0.6.0–v1.0.
...
- **Long-press action** — optional secondary action, with threshold ms
...
- All built-in actions in the table above (those that depend on transport / tap tempo are stubs until v0.8.0)
- Learn flow for hardware MIDI binding
- Configurable widget for Perform mode (basic styling — full editor in v0.10.0)
