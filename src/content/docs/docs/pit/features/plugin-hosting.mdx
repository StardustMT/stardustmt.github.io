...
<Badge text="✅ Shipped · v0.5.0 · sandboxing v0.7.0" variant="success" />
...
- Plugin GUI displays in the patch editor's **bottom GUI dock** by default (tab strip per loaded plugin in the active patch); pop-out button moves it to a floating window per the user's per-plugin preference. Per-plugin placement persists across patch reloads. See [locked decisions](/docs/pit/decisions/).
