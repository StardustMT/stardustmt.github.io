...
Most existing live hosts run plugins in-process for performance reasons, which means a single plugin crash takes the host down with it. Stardust accepts the ~1 ms IPC overhead to keep that from happening. See [Comparison](/comparison/) for how this compares across hosts, and **Plugin Sandboxing** <!-- TODO: dead wiki link to 'Architecture: Plugin Sandboxing' --> for IPC and process-model details.
...
See [Latency Budget](/reliability/latency-budget/).

## Plugin parameter handling

Once a plugin is loaded:
- Every parameter is exposed in the Patch editor
- Parameters can be mapped to MIDI controls via [MIDI Learn](/features/midi-learn/)
...
See [Audio I/O](/features/audio-io/) for related setup.
...
- [Crash Isolation](/reliability/plugin-crash-isolation/)
